...
/**
 * The AFPParserConfiguration is used to configure the {@link AFPParser}, see {@link AFPParser#AFPParser(AFPParserConfiguration)}.
 */
public class AFPParserConfiguration implements Serializable,Cloneable{
	...
	/**
	 * Returns the {@link Charset} used to decode text contained in the AFP data stream (e.g. PTX,NOP,TLE). 
	...
	/**
	 * Sets the {@link Charset} used to decode text contained in the AFP data stream (e.g. PTX,NOP,TLE).
	 ...
	 * Default is 500KB. Minimum is 100KB.
	 * A higher value can result in higher performance of the parser.
	...
	/**
	 * Returns the {@link InputStream} from where the parser reads the AFP data stream.
	 * If no input stream is set this method tries to open the configured AFP file (see {@link #setAFPFile(File)})
	 * as buffered input stream.<br>
	 * @return the {@link InputStream} from where the parser reads the AFP data stream.
	 * @throws IOException if the input stream is not set and the opening of the configured AFP file could not be opened. 
	 */
	...
	/**
	 * Returns true if the parser decodes structured fields 
	 * of type {@link StructuredFieldBaseData} and the payload of the structured field
	 * is seen as blackbox and not further processed by the parser.
	 ...
	 * that provides only getter and setter for the structured fields's payload.<br>
	 * <br>
	 * This mode is especially usefull when dealing with AFP data that isn't fully compliant to AFP standards.
	 * In this mode, the parser is less restrictive, e.g. doesn't care if the length of the 
	 * structured field or individual attribute values are valid according to AFP specifications.
	 * 
	 * @param isParseToStructuredFieldsBaseData
	 */
	...
	/**
	 * If set to true the parser is building shallow {@link StructuredField}s.
	 * See {@link #isBuildShallow()}. 
	 * @param isBuildShallow
	 */
