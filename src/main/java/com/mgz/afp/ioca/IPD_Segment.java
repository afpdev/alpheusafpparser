...
abstract class IPD_Segment implements IAFPDecodeableWriteable {
	...
	public IPD_Segment.IPD_SegmentType getSegmentType() { return segmentType;}
	public void setSegmentType(IPD_Segment.IPD_SegmentType segmentType) {this.segmentType = segmentType;}
	public int getLengthOfFollowingData() {return lengthOfFollowingData;}
	public void setLengthOfFollowingData(int lengthOfFollowingData) {this.lengthOfFollowingData = lengthOfFollowingData;}
	...
	public static class IDEStructure extends IPD_Segment.IPD_SegmentLong{
		public static enum IDEStructureFlag implements IMutualExclusiveGroupedFlag{
			...
			;
			int group;
			IDEStructureFlag(int group){ this.group = group;}
			@Override
	...
	public static class ExternalAlgorithmSpecification extends IPD_Segment.IPD_SegmentLong{
		public static enum AlgorithmType{
			...
			Compressing(0x10);
			int code;
			AlgorithmType(int code){this.code=code;}
	...
	public abstract static class AlgorithmSpecificationCompression extends IPD_Segment.AlgorithmSpecification{
		public static enum CompressionAlgorithmID{
			...
			Userdefined(0xFE);
			int code;
			CompressionAlgorithmID(int code){this.code=code;}
		...
		public AlgorithmSpecificationCompression.CompressionAlgorithmID getCompressionAlgorithmID() { return compressionAlgorithmID;}
		public void setCompressionAlgorithmID( AlgorithmSpecificationCompression.CompressionAlgorithmID compressionAlgorithmID) { this.compressionAlgorithmID = compressionAlgorithmID; }
	}
	...
	public static class JPEGCompressionAlgorithmSpecification extends IPD_Segment.AlgorithmSpecificationCompression{
		public static enum JPEGCompressionAlgorithmSpecificationMarker{
			...
			int code;
			JPEGCompressionAlgorithmSpecificationMarker(int code){this.code=code;}
			public static JPEGCompressionAlgorithmSpecificationMarker valueOf(short codeByte) {
				for(JPEGCompressionAlgorithmSpecificationMarker marker : values()) if(marker.code==codeByte) return marker;
				return null;
			}
			public int toByte() { return code;}
		}
	...
	public static class ImageSubsampling extends IPD_Segment.IPD_SegmentExtended{
		...
		public static abstract class ImageSubsamplingField implements IAFPDecodeableWriteable{
			...
			public short getFieldType(){return fieldType;}
			public void setFieldType(short fieldType){this.fieldType = fieldType;}
			...
			public static class SamplingRatios extends ImageSubsampling.ImageSubsamplingField{
				...
				public static class SamplingRatiosRepeatingGroup{
					...
					public byte getNrOfHorizontalSamples() { return nrOfHorizontalSamples; }
					public void setNrOfHorizontalSamples(byte nrOfHorizontalSamples) { this.nrOfHorizontalSamples = nrOfHorizontalSamples;}
					public byte getNrOfVerticalSamples() {return nrOfVerticalSamples;}
					public void setNrOfVerticalSamples(byte nrOfVerticalSamples) {this.nrOfVerticalSamples = nrOfVerticalSamples;}
				}
	...
	public static class TileSize extends IPD_Segment.IPD_SegmentLong{
		public static enum RelativeTileResolution{
			...
			int code;
			RelativeTileResolution(int code){ this.code=code; }
	...
	public static class TileTOC extends IPD_Segment.IPD_SegmentExtended{
		...
		@Override
		public void writeAFP(OutputStream os, AFPParserConfiguration config) throws IOException {
			if(listOfRepeatingGroups==null || listOfRepeatingGroups.isEmpty()) lengthOfFollowingData=2;
			else lengthOfFollowingData = 2 + listOfRepeatingGroups.size()*26;
			os.write(segmentType.toBytes());
			os.write(UtilBinaryDecoding.intToByteArray(lengthOfFollowingData,2));
			os.write(reserved4_5);
			if(listOfRepeatingGroups!=null){
				for(TileTOC.TileTOC_RepeatingGroup rg: listOfRepeatingGroups)rg.writeAFP(os, config);
			}
		}
		...
		public static class TileTOC_RepeatingGroup implements IAFPDecodeableWriteable{
			...
			/**
			 * Note that the IOCA specifies an unsigned 8byte value for {@link #offsetInBytesFromBeginSegmentToBeginTile} 
			 * which is in fact an quite astronomical number.
			 * This method however support only signed 8byte values (long) which is about 1.84467*10^19 (2^64-1) bytes which 
			 * is in fact only half the size as specified in IOCA but is still large enough to cover Image Picture Data in the 
			 * range of millions of terrabyte.<br>
			 * 
			 * @param offsetInBytesFromBeginSegmentToBeginTile
			 */
	...
	public static enum IPD_SegmentType{
		...
		int type;
		IPD_SegmentType(int type){ this.type=type; }
	...
	public static enum IPD_CompressionAlgorithm{
		...
		int type;
		IPD_CompressionAlgorithm(int type){ this.type=type; }
		public static IPD_Segment.IPD_CompressionAlgorithm valueOf(short segmentTypeByte){
			for(IPD_Segment.IPD_CompressionAlgorithm t : values()) if(t.type == segmentTypeByte) return t;
			return null;
		}
		public int toByte(){ return type; }
	}
	...
	public static enum IPD_RecordingAlgorithm{
		...
		int type;
		IPD_RecordingAlgorithm(int codeByte){this.type = codeByte;}
		public static IPD_Segment.IPD_RecordingAlgorithm valueOf(short segmentTypeByte){
			for(IPD_Segment.IPD_RecordingAlgorithm t : values()) if(t.type == segmentTypeByte) return t;
			return null;
		}
		public int toByte(){ return type; }
	}
	...
	public static enum IPD_BitOrder{
		...
		RightToLeft(0x01);
		int type;
		IPD_BitOrder(int codeByte){this.type = codeByte;}
