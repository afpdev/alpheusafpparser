...
public class CPC_CodePageControl extends StructuredField {
	...
	/** "This parameter is EBCDIC encoded using CPGID 500 (International #5) and GCSGID 103 (International DP 94 (ASCII))." */
	@AFPField
	...
	public static enum DefaultCharacterUseFlag{
		/** When this flag is set, the character is an invalid coded character (see “Invalid Coded Character” on page 112). When this flag bit is set to B'0' (off), the character is a valid coded character. */
		InvalidCodedCharacter,
		/** When this flag is set, the character not to be printed (see “No Presentation” on page 113). When this flag bit is set to B'0' (off), the character is to be printed. */
		NoPresentation,
		/** When this flag bit is set, the print position is not to be incremented (see “No Increment” on page 112). When this flag bit is set to B'0' (off), the print position is to be incremented. */
		NoIncrement;
	...
	public static enum CPIRepeatingGroupLength{
		...
		public short nrOfBytes(){
			if( this == DoubleByteCodePage || this == DoubleByteCodePageUnicodeScalarValues) return 2;
			else return 1;
		}
		public boolean isUnicodeScalarValues(){
			if( this == SingleByteCodePageUnicodeScalarValues || this == DoubleByteCodePageUnicodeScalarValues) return true;
			else return false;
		}
	}
	...
	public static enum CodePageUseFlag{
		...
		SortOrder,
		/** This flag is used to enable variable spacing. When the flag is set, the PTOCA variable space increment is used whenever the code point for the variable space character is encountered. Some AFP products, such as DCF, use variable spacing for text justification and therefore require this flag to be set. When the flag is not set, all PTOCA SVI control sequences are ignored and the space character is printed as defined in the accompanying font whenever the variable space code point is encountered. If a code page is built that has this flag not set, it should not be used with AFP products that require the variable space function. */
		VariableSpaceEnable;
