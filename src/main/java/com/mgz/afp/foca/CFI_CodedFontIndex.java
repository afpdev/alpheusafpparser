...
/**
 * <b>CFI – D38C8A – Coded Font Index</b><br>
 * The Coded Font Index (CFI) identifies one or more Font Character Set and Code Page object pairs, and associated size information for outline font scaling.<br>
 * <br>
 * CFI structured field names the font character sets and
 * code pages for the font. The structured field contains a set of parameters, defined
 * as a repeating group. The length of the repeating group is defined in the Coded
 * Font Control structured field. The number of repeating groups in the structured
 * field can be determine by dividing the length of the CFI structured field (minus
 * the length of the structured field introducer) by the length of the CFI repeating
 * group. The repeating groups are sorted in ascending order based on the section
 * identifier.<br>
 * <br>
 * <b>Single-Byte and Double-byte Outline</b><br>
 * There is only one section for a single-byte font, or a double-byte outline font;
 * therefore, there is only one repeating group.<br>
 * <br>
 * <b>Double-Byte Raster Coded Fonts</b><br>
 * There can be 190 sections (X'41' to X'FE'). Each section requires its own
 * repeating group. The repeating groups are sorted in ascending order based on
 * the section identifier.<br>
 * <br>
 * The data for the CFI structured field consists of a series of repeating groups.
 */
public class CFI_CodedFontIndex extends StructuredField {
	...
	public static class CFIRepeatingGroup implements IAFPDecodeableWriteable{
		...
		@AFPField
		int shScale;
		/**  Section Number:<br><b>0x00</b> Single-byte<br><b>0x41 – 0xFE</b> Double-byte Raster<br><b>0xFF</b> reserved. */
		@AFPField
		short section;
	...
	public void addCFIRepeatingGroup(CFIRepeatingGroup CFIrg){
		...
		if(this.cfiRepeatingGroups==null) this.cfiRepeatingGroups=new ArrayList<CFIRepeatingGroup>();
