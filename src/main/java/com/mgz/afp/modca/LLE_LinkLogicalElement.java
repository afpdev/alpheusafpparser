...
/**
 * <b>LLE_LinkLogicalElement</b> (MO:DCA, page 222)<br>
 * A Link Logical Element structured field specifies the linkage from a source 
 * document component to a target document component. The LLE identifies the 
 * source and target and indicates the purpose of the linkage by specifying a link
 * type. The link source and link target may be in the same document component or
 * in different document components, and they need not be of the same document
 * component type. The linkage may involve a complete document component, or it
 * may be restricted to a rectangular area on the presentation space associated with
 * the document component. The Link Logical Element structured field can be
 * embedded in the document that contains the link source, in the document that
 * contains the link target, in the document index for either document, or in any
 * combination of these structures. Link Logical Element parameters do not provide
...
public class LLE_LinkLogicalElement extends StructuredField {
	...
	public void addRepeatingGroup(LLE_RepeatingGroup repeatingGroup){
		...
		if(repeatingGroups==null) repeatingGroups = new ArrayList<LLE_LinkLogicalElement.LLE_RepeatingGroup>();
	...
	public static class LLE_RepeatingGroup implements IAFPDecodeableWriteable, IHasTriplets{
		public static enum LLE_RepeatingGroupFunction{
			...
			public static LLE_RepeatingGroupFunction valueOf(byte linkTypeByte){
				for(LLE_RepeatingGroupFunction lt : values()) if(lt.ordinal()==(linkTypeByte-1)) return lt;
				return null;
			}
			public int toByte(){ return this.ordinal()+1; }
		}		
