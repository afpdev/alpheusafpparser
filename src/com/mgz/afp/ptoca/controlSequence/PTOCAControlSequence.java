...
public abstract class PTOCAControlSequence implements IAFPDecodeableWriteable{
	...
	public static class ControlSequenceIntroducer{
		...
		/**
		 * If true the NEXT control sequence that follows thsi control sequence has a control sequence identifier in the "chained" format.
		 * @return
		 */
	...
	public static enum ControlSequenceFunctionType{
		...
		int typeCode;
		ControlSequenceFunctionType(int typeCode){this.typeCode = typeCode;}
		...
		public static ControlSequenceFunctionType valueOf(short typeCode) throws AFPParserException{
			for(ControlSequenceFunctionType csft : values()) if (csft.typeCode == typeCode || csft.typeCode==(typeCode-1)) return csft;
			return Undefined;
		}
	}
	...
	/** PTOCA, Page 51. <br>The Absolute Move Baseline control sequence moves the baseline coordinate relative to the I-axis. */
	...
	/** PTOCA, Page 53. <br>The Absolute Move Inline control sequence moves the inline coordinate position relative to the B-axis. */
	...
	/** PTOCA, Page 56. <br>The Begin Suppression control sequence marks the beginning of a string of presentation text that may be suppressed from the visible output. */
	...
	public static class TRN_TransparentData extends PTOCAControlSequence{
		...
		/**
		 * If {@link #isUseEBCDICData()} is set tot true the EBCDIC data is written out without performing an encoding/decoding round trip.
		 * See {@link #setUseEBCDICData(boolean)}.
		 */
		@Override
		public void writeAFP(OutputStream os, AFPParserConfiguration config) throws IOException {
		...
		/**
		 * Returns true if the EBCDIC data is used in {@link #writeAFP(OutputStream, AFPParserConfiguration)} 
		 * without performing a encoding/decoding round trip.
		 * @return
		 */
		...
		/**
		 * If given parameter is true the EBCDIC data is used in {@link #writeAFP(OutputStream, AFPParserConfiguration)}  
		 * without performing an encoding/decoding round trip.<br>
		 * This might be usefull if problems occure with special code points (eg. user defined characters, variable 
		 * space character) that can't be properly mapped to Unicode characters and therefore, when encoded back to EBCDIC,
		 * may produce unwanted results.  
		 * @param isUseEBCDICData
		 */
