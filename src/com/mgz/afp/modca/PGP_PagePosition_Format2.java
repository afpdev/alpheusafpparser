 ...
 * MO:DCA, page 312.
 * <br><br>
The Page Position structured field specifies the position and orientation of a page's
presentation space on the medium presentation space for the physical medium.
The PGP may be located in a medium map or in the document environment group
of a form map. When present in the active medium map, it overrides a PGP in the
document environment group of the form map. If N-up partitioning is specified by
the Medium Modification Control structured field in the active medium map, the
medium presentation spaces on the front and back sides of a sheet are divided into
N partitions; and the Page Position structured field specifies the partition into
which each page is mapped and with respect to which the page presentation space
is positioned and oriented. The N-up page-to-partition mapping can be specified in
two mutually exclusive ways:
<ul>
<li>Default N-up page placement. Pages are processed in the order in which they
appear in the data stream and are placed into consecutively-numbered
partitions, that is, the first page is placed into partition 1, the second page is
placed into partition 2, the third page is placed into partition 3, and the 4th page
is placed into partition 4. Partition numbering for various media is shown in
Figure 58 on page 320 to Figure 69 on page 326.
<li> Explicit N-up page placement. Pages are processed in the order in which they
appear in the data stream and are placed into the partition that is explicitly
specified by the repeating group for the page. Multiple pages may be placed
into the same partition. If N-up simplex is specified, the Page Position structured
field must contain N repeating groups, one for each page on the sheet-side. If
N-up duplex is specified, the Page Position structured field must contain 2N
repeating groups, one for each page on the sheet.
</ul>
 */
public class PGP_PagePosition_Format2 extends StructuredFieldBaseRepeatingGroups {
	...
	public static class PGP_RepeatingGroup extends RepeatingGroupBase {
		...
		public static enum PGP_SheetSideAndPartitionSelection{
			...
			int code;
			PGP_SheetSideAndPartitionSelection(int code){this.code = code;}
			public static PGP_SheetSideAndPartitionSelection valueOf(byte codeByte){
				for(PGP_SheetSideAndPartitionSelection ssps : values()) if(ssps.code == codeByte) return ssps;
				return null;
			}
		...
		public static enum PGP_RGFlag implements IMutualExclusiveGroupedFlag{
			...
			int group;
			PGP_RGFlag(int group){this.group=group;}
			@Override
