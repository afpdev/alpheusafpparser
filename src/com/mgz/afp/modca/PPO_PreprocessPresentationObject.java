...
parameters for a data object that has been mapped as a resource. These parameters
allow the presentation device to preprocess and cache the object so that it is in
presentation-ready format when it is included with a subsequent include
structured field in the document. Such preprocessing may involve a rasterization
or RIP of the object, but is not limited to that. The resource is identified with a file
name, the identifier of a begin structured field for the resource, or any other
identifier associated with the resource. The referenced resource and all required
secondary resources must previously have been mapped with an MDR or an MPO
in the same environment group.
 */
public class PPO_PreprocessPresentationObject extends StructuredFieldBaseRepeatingGroups {
	...
	public static class PPO_RepeatingGroup extends RepeatingGroupWithTriplets{
		...
		public static enum PPO_Flag implements IMutualExclusiveGroupedFlag{
			...
			PPO_Flag(int group){this.group = group;}
			...
			public static void setFlag( EnumSet<PPO_Flag> flags, PPO_Flag flag){ handler.setFlag(flags, flag); }
		}
