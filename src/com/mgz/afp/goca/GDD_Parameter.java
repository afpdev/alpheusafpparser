...
public abstract class GDD_Parameter implements IAFPDecodeableWriteable{
	...
	/**
	 * Returns the type code of this {@link GDD_Parameter}.
	 * This values is set by the sub-type of {@link GDD_Parameter}.
	 * @return type code of this {@link GDD_Parameter}.
	 */
	public short getParameterType() { return parameterType; }
	 ...
	 */
	public short getLengthOfFollowingData() { return lengthOfFollowingData;}
	public void setLengthOfFollowingData(short lengthOfFollowingData) {this.lengthOfFollowingData = lengthOfFollowingData;}	

	
	
	/**
	 * GOCA, page 161.<br><br>
	 * Defaults can be set by the appropriate Set Current Defaults instructions. For a
		complete description of this instruction, see “Set Current Defaults (SCD)
		Instruction” on page 60. Each occurrence of the Set Current Defaults instruction
		specifies a particular attribute set. The following tables show the maximum set of
		attributes allowed. Subsets of these attribute sets are also allowed, using the MASK
		bits as selectors for attributes in the particular attribute set. The format of the
		attribute sets is described in “Set Current Defaults (SCD) Instruction” on page 60.
		In the tables below, two possibilities exist. If the FLAG byte equals X'8F', the
		LENGTH byte would be specified as the second value shown, and the values
		shown in bytes 6-n would be specified as shown. If the FLAG byte instead equals
		X'0F', the LENGTH byte would be specified as the first value shown (that is, 4),
		and bytes 6-n would not be specified.
	 */
	public abstract static class SetCurrentDefaultInstruction extends GDD_Parameter{
		...
		public static class DrawingAttributes extends SetCurrentDefaultInstruction{
			...
			public void writeAFP(OutputStream os, AFPParserConfiguration config) throws IOException {
				...
				if(flag==Flag.UseValuesAsSpecified){
					...
					if(mask.get(0)) baos.write(color!=null ? color.toByte2() : AFPColorValue.DeviceDefault_0xFF00.toByte2());
					if(mask.get(2)) baos.write(foregroundMix!=null ? foregroundMix.toByte() : 0x00);
					if(mask.get(3)) baos.write(backgroundMix!=null ? backgroundMix.toByte() : 0x00);
		...
		public static class LineAttributes extends SetCurrentDefaultInstruction{
			...
			public void writeAFP(OutputStream os, AFPParserConfiguration config) throws IOException {
				...
				if(flag==Flag.UseValuesAsSpecified){
					ByteArrayOutputStream baos = new ByteArrayOutputStream();
					if(mask.get(0)) baos.write(lineType !=null ? lineType.toByte() : LineType.Default.toByte());
					if(mask.get(1)) baos.write(lineWidth !=null ? lineWidth : 0x00);
					if(mask.get(2)) baos.write(lineEnd !=null ? lineEnd.toByte() : LineEnd.Default.toByte());
					if(mask.get(3)) baos.write(lineJoin !=null ? lineJoin.toByte() : LineJoin.Default.toByte());
					attributeData = baos.toByteArray();
				}
		...
		public static class CharacterAttributes extends SetCurrentDefaultInstruction{
			...
			public void writeAFP(OutputStream os, AFPParserConfiguration config) throws IOException {
				...
				if(flag==Flag.UseValuesAsSpecified){
					ByteArrayOutputStream baos = new ByteArrayOutputStream();
					if(mask.get(0)) baos.write(characterAngleXY!=null ? UtilBinaryDecoding.longToByteArray(characterAngleXY,4) : Constants.EMPTYBYTES_4);
					if(mask.get(1)) baos.write(characterCellSizeWH!=null ? UtilBinaryDecoding.longToByteArray(characterCellSizeWH,4): Constants.EMPTYBYTES_4);
					if(mask.get(2)) baos.write(characterDirection!=null ? characterDirection : 0x00);
					if(mask.get(3)) baos.write(characterPrecision!=null ? characterPrecision : 0x00);
					if(mask.get(4)) baos.write(characterSet!=null ? characterSet : 0x00);
					if(mask.get(5)) baos.write(characterShearXY!=null ? UtilBinaryDecoding.longToByteArray(characterShearXY,4) : Constants.EMPTYBYTES_4);
					attributeData = baos.toByteArray();
				}
				lengthOfFollowingData = attributeData!=null ? (short)(4 + attributeData.length) : 4;
				super.writeAFP(os, config);
		...
		public static class ArcParameters extends SetCurrentDefaultInstruction{
			...
			public void writeAFP(OutputStream os, AFPParserConfiguration config) throws IOException {
				...
				if(flag==Flag.UseValuesAsSpecified){
					ByteArrayOutputStream baos = new ByteArrayOutputStream(); 
					if(mask.get(0)) baos.write(arcTransformP!=null ? UtilBinaryDecoding.shortToByteArray(arcTransformP, 2) : Constants.EMPTYBYTES_2);
					if(mask.get(1)) baos.write(arcTransformQ!=null ? UtilBinaryDecoding.shortToByteArray(arcTransformQ, 2) : Constants.EMPTYBYTES_2);
					if(mask.get(2)) baos.write(arcTransformR!=null ? UtilBinaryDecoding.shortToByteArray(arcTransformR, 2) : Constants.EMPTYBYTES_2);
					if(mask.get(3)) baos.write(arcTransformS!=null ? UtilBinaryDecoding.shortToByteArray(arcTransformS, 2) : Constants.EMPTYBYTES_2);
					attributeData = baos.toByteArray();
				}
				lengthOfFollowingData = attributeData!=null ? (short)(4 + attributeData.length) : 4;
				super.writeAFP(os, config);
		...
		public static class ProcessColorAttributes extends SetCurrentDefaultInstruction{
			...
			public void writeAFP(OutputStream os, AFPParserConfiguration config) throws IOException {
				...
				if(flag==Flag.UseValuesAsSpecified){
					ByteArrayOutputStream baos = new ByteArrayOutputStream();
					if(mask.get(0)) baos.write(foregroundMix !=null ? foregroundMix.toByte() : AFPForegroundMix.Default.toByte());
					if(mask.get(1)) baos.write(backgroundMix != null ? backgroundMix.toByte() : AFPBackgroundMix.Default.toByte());
					if(mask.get(2)) baos.write(processColor != null ? processColor : Constants.EMPTYBYTES_3);
					attributeData = baos.toByteArray();
				}
				lengthOfFollowingData = attributeData!=null ? (short)(4 + attributeData.length) : 4;
	...
	public static class WindowSpecification extends GDD_Parameter{
		...
		public enum WindowSpecificationFlag implements IMutualExclusiveGroupedFlag{
			...
			int group;
			WindowSpecificationFlag(int group){ this.group=group; }
			@Override
			...
			public static EnumSet<WindowSpecification.WindowSpecificationFlag> valueOf(byte flagByte){
				...
				if((flagByte & 0x10)==0) result.add(PictureResoulution_NotDefined_or_NonSymetric);
				else result.add(PictureResoulution_XYEqual_DefinedByIMXYRES);
				if((flagByte & 0x08)==0) result.add(SymetricImage_PictureResoulution_NotDefined_or_Symetric);
			...
			public static int toByte(EnumSet<WindowSpecification.WindowSpecificationFlag> controlFlags){
				...
				if(controlFlags.contains(PictureResoulution_XYEqual_DefinedByIMXYRES)) result|=0x10;
				if(controlFlags.contains(SymetricImage_PictureResoulution_120x140dpi)) result|=0x08;
