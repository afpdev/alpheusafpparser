 ...
 * GOCA, 164.<br><br>
 * 
	The graphics segments for a graphics object are contained within one or more
	GAD structured fields. Receipt of the first segment starts the drawing process. No
	restrictions exist on how much or how little graphics data is specified in a single
	GAD, except for the length limit of the structured field. A GAD, for example, can
	carry partial segments, full segments, multiple segments, or any combination of
	these. The only requirement is that the data itself is ordered in the sequence that is
	expected for immediate processing and that the last GAD completes the last
	segment.
	Because this environment does not support the calling of segments, all segments
	should be chained segments. Any unchained segments in the data are ignored.
	The GAD structured field is optional in a MO:DCA graphics object and may be
...
public class GAD_GraphicsData extends StructuredField {
	...
	public static class BeginSegment implements IAFPDecodeableWriteable{
		...
		public void writeAFP(OutputStream os, AFPParserConfiguration config) throws IOException {
			...
			if(segmentPropertiesFlags!=null) os.write(SegmentPropertiesFlag.toByte(segmentPropertiesFlags));
			else os.write(0x00);
		...
		public static enum SegmentPropertiesFlag implements IMutualExclusiveGroupedFlag{
			...
			int group;
			SegmentPropertiesFlag(int group){this.group = group;}
			@Override
		...
		/**
		 * Sets the given {@link SegmentPropertiesFlag} and un-sets corresponding mutual exclusive flags.
		 * @param flag {@link SegmentPropertiesFlag} to set.
		 */
		public void setSegmentPropertiesFlag(SegmentPropertiesFlag flag){
			if(segmentPropertiesFlags==null) segmentPropertiesFlags = EnumSet.noneOf(SegmentPropertiesFlag.class);
	...
	private static final List<GAD_DrawingOrder> buildDrawingOrders(byte[] sfData, int offset, int length, AFPParserConfiguration config) throws AFPParserException {
		...
		int dotLength = 0;
			...
			switch(drawingOrderCode){
				case 0x00: { drawingOrder = new GNOP1_NopOperation(); dotLength=1;} break;
				case 0x01: { drawingOrder = new GCOMT_Comment(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0x04: { drawingOrder = new GDGCH_SegmentCharacteristics(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0x08: { drawingOrder = new GSPS_SetPatternSet(); dotLength=2;} break;
				case 0x0a: { drawingOrder = new GSCOL_SetColor(); dotLength=2;} break;
				case 0x0c: { drawingOrder = new GSMX_SetMix(); dotLength=2;} break;
				case 0x0d: { drawingOrder = new GSBMX_SetBackgroundMix(); dotLength=2;} break;
				case 0x11: { drawingOrder = new GSFLW_SetFractionLineWidth(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0x18: { drawingOrder = new GSLT_SetLineType(); dotLength=2;} break;
				case 0x19: { drawingOrder = new GSLW_SetLineWidth(); dotLength=2;} break;
				case 0x1a: { drawingOrder = new GSLE_SetLineEnd(); dotLength=2;} break;
				case 0x1b: { drawingOrder = new GSLJ_SetLineJoin(); dotLength=2;} break;
				case 0x21: { drawingOrder = new GSCP_SetCurrentPosition(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0x22: { drawingOrder = new GSAP_SetArcParameters(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0x26: { drawingOrder = new GSECOL_SetExtendedColor(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0x28: { drawingOrder = new GSPT_SetPatternSymbol(); dotLength=2;} break;
				case 0x29: { drawingOrder = new GSMT_SetMarkerSymbol(); dotLength=2;} break;
				case 0x33: { drawingOrder = new GSCC_SetCharacterCell(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0x34: { drawingOrder = new GSCA_SetCharacterAngle(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0x35: { drawingOrder = new GSCH_SetCharacterShear(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0x37: { drawingOrder = new GSMC_SetMarkerCell(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0x38: { drawingOrder = new GSCS_SetCharacterSet(); dotLength=2;} break;
				case 0x39: { drawingOrder = new GSCR_SetCharacterPrecision(); dotLength=2;} break;
				case 0x3a: { drawingOrder = new GSCD_SetCharacterDirection(); dotLength=2;} break;
				case 0x3b: { drawingOrder = new GSMP_SetMarkerPrecision(); dotLength=2;} break;
				case 0x3c: { drawingOrder = new GSMS_SetMarkerSet(); dotLength=2;} break;
				case 0x3e: { drawingOrder = new GEPROL_EndProlog(); dotLength=2;} break;
				case 0x60: { drawingOrder = new GEAR_EndArea(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0x68: { drawingOrder = new GBAR_BeginArea(); dotLength=2;} break;
				case 0x80: { drawingOrder = new GCBOX_BoxAtCurrentPosition(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0x81: { drawingOrder = new GCLINE_LineAtCurrentPosition(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0x82: { drawingOrder = new GCMRK_MarkerAtCurrentPosition(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0x83: { drawingOrder = new GCCHST_CharacterStringAtCurrentPosition(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0x85: { drawingOrder = new GCFLT_FilletAtCurrentPosition(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0x87: { drawingOrder = new GCFARC_FullArcAtCurrentPosition(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0x91: { drawingOrder = new GCBIMG_BeginImageAtCurrentPosition(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0x92: { drawingOrder = new GIMD_ImageData(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0x93: { drawingOrder = new GEIMD_EndImage(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0xa1: { drawingOrder = new GCRLINE_RelativeLineAtCurrentPosition(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0xa3: { drawingOrder = new GCPARC_PartialArcAtCurrentPosition(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0xa5: { drawingOrder = new GCCBEZ_CubicBezierCurveAtCurrentPosition(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0xb2: { drawingOrder = new GSPCOL_SetProcessColor(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0xc0: { drawingOrder = new GBOX_BoxAtGivenPosition(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0xc1: { drawingOrder = new GLINE_LineAtGivenPosition(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0xc2: { drawingOrder = new GMRK_MarkerAtGivenPosition(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0xc3: { drawingOrder = new GCHST_CharacterStringAtGivenPosition(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0xc5: { drawingOrder = new GFLT_FilletAtGivenPosition(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0xc7: { drawingOrder = new GFARC_FullArcAtGivenPosition(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0xd1: { drawingOrder = new GBIMG_BeginImageAtGivenPosition(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0xe1: { drawingOrder = new GRLINE_RelativeLineAtGivenPosition(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0xe3: { drawingOrder = new GPARC_PartialArcAtGivenPosition(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0xe5: { drawingOrder = new GCBEZ_CubicBezierCurveAtGivenPosition(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +1 , 1) + 2;} break;
				case 0xfe: { drawingOrder = new GEXO_ExtendedOrder(); dotLength=UtilBinaryDecoding.parseInt(sfData, offset + pos +2 , 2) + 4;} break;
				default: { drawingOrder = null; } break;
			}
