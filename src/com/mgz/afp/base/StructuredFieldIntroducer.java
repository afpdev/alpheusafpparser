...
public class StructuredFieldIntroducer {
	...
	/** ExtData[9-263] Optional. Contains up to 254 bytes of application-defined SFI extension data. For ExtData to be valid, bit 0 of FlagByte must be B'1'.<br> A structured field introducer may be extended by up to 255 bytes. The presence of an SFI extension is indicated by a value of B'1' in bit 0 of the SFI flag byte. If an extension is present, the introducer is at least 8 bytes, but not more than 263 bytes, in length. The first byte of the extension specifies its length. If an extension to the structured field introducer is present, the structured field's data can occupy up to 32,759 bytes, less the length of the extension. */
	...
	public byte[] toBytes(){
		...
		if(flagByte==null || !flagByte.contains(SFFlag.hasExtension)) b = new ByteArrayOutputStream(8);
	 ...
	 * The corresponding {@link SFFlag#hasExtension} flag and size of extender data of SFI is set accordingly.
	 * If the given extender data is null, the {@link SFFlag#hasExtension} flag is revoked and size of extender data of SFI is set to zero (not used).
	 * @param sfiExtensionData
	 */
	 ...
	 * A structured field begins with the 0x5A byte.
	 * @return
	 */
