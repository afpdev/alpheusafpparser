...
public abstract class Triplet implements IAFPDecodeableWriteable{
	...
	public TripletID getTripletID() { return tripletID; }
	public void setTripletID(TripletID tripletID) { this.tripletID = tripletID;}	
	...
	public static enum TripletID{
		...
		/**
		 * MODCA page 379.<br><br>
		 * The Resource Object Type triplet identifies the type of object enveloped by the
		 * Begin Resource (BRS) and End Resource (ERS) structured fields.<br>
		 * <br>
		 * <b>UNFORTUNATLY:</b><br>
		 * A similar triplet, the Object Function Set Specification triplet,
			that <b>unfortunately also uses triplet ID X'21'</b>, is retired but is still
			used on the BDT structured field; see “Object Function Set
			Specification Triplet X'21'” on page 570.
		 */
		...
		;
		int code;
		TripletID(int code){ this.code= code;}
	...
	/**
	 * The {@link Undefined} triplet is used when problem occur with corrupted AFP Data.
	 * The {@link #tripletData} field contains the entire of the data.
	 * The {@link TripletID} field contains {@link TripletID#Undefined} and is 
	 * not read from the data given to {@link #decodeAFP(byte[], int, int, AFPParserConfiguration)},
	 * nor is it written by {@link #writeAFP(OutputStream, AFPParserConfiguration)}.
	 * <br>
	 * <br>
	 * The method {@link #writeAFP(OutputStream, AFPParserConfiguration)} simple writes the data contained
	 * in field {@link #tripletData}. In addition it resets the value of  the length field.
	 */
	...
	/**
	 * MO:DCA, page 349.<br><br> 
The Coded Graphic Character Set Global Identifier (CGCSGID) triplet is used to
establish the values of the code page and character set for interpretation of all
structured field parameters having a CHAR data type, such as name parameters,
except where such parameters define a fixed encoding. An example of a parameter
that defines its own encoding is the character string specified with a Fully
Qualified Name (X'02') triplet using FQNFmt = X'20' - URL, which is encoded
using the US-ASCII coded character set.
	 */
	...
	/**
	 * MO:DCA, page 353.<br><br> 
	 * The Fully Qualified Name triplet enables the identification and referencing of
	 * objects using Global Identifiers (GIDs).
	 */
	...
	/** MO:DCA, page 365.<br><br> The Mapping Option is used to specify the mapping of a data object presentation space to an object area. */
	public static class MappingOption extends Triplet {
		...
		public static enum DataObjecMapingOption{
			...
			int code;
			DataObjecMapingOption(int code){this.code=code;}
	...
	/** MO:DCA, page 368.<br><br> The Object Classification is used to classify and identify object data. The object data may or may not be defined by an AFP presentation architecture. */
	public static class ObjectClassification extends Triplet {
		...
		/** Sets the given {@link StructureFlag} and unsets mutual exclusive flags in {@link #structureFlags}. */
		public void setStructureFlag(StructureFlag flag){
			StructureFlag.setFlag(structureFlags, flag);
		}


		/**  Specifies the object class based on differentiators such as temporal characteristics and presentation form. */
		public static enum ObjectClass{
			...
			DataObjectFont(0x41);
			int code;
			ObjectClass(int code){ this.code=code; }
		...
		public static enum StructureFlag implements IMutualExclusiveGroupedFlag{
			...
			int group;
			StructureFlag(int code){ this.group=code; }
			public static EnumSet<StructureFlag> valueOf(int codeByte){
				...
				else if((codeByte & 0x8000)!=0 && (codeByte & 0x4000)!=0) result.add(OC_DataCarriedInObjectContainer);
				...
				else if((codeByte & 0x2000)!=0 && (codeByte & 0x1000)!=0) result.add(OEG_Included);
				...
				else if((codeByte & 0x0800)!=0 && (codeByte & 0x0400)!=0) result.add(OCD_DataCarriedInOCD);
	...
	/** MO:DCA, page 372.<br><br> The MO:DCA Interchange Set triplet identifies the interchange set and the data stream type. */
	public static class MODCAInterchangeSet extends Triplet {
		...
		public static enum MODCAInterchangeSet_Identifier{
			...
			MODCA_IS3(0x0D00);
			int code;
			MODCAInterchangeSet_Identifier(int code){this.code=code;}
	...
	/** MO:DCA, page 374.<br><br> The Font Descriptor Specification triplet specifies the attributes of the desired font in a coded font reference. */
	public static class FontDescriptorSpecification extends Triplet {
		...
		public static enum FDS_FontWeigthClass{
			...
			UltraBold(0x09);
			int code;
			FDS_FontWeigthClass(int code){this.code=code;}
		...
		public static enum FDS_FontWidthClass{
			...
			Ultra_Expanded(0x09);
			int code;
			FDS_FontWidthClass(int code){this.code=code;}
		...
		public static enum FDS_FontDsFlag implements IMutualExclusiveGroupedFlag{
			...
			int group;
			FDS_FontDsFlag(int group){this.group = group;}
		...
		public static enum FDS_FontUsFlag implements IMutualExclusiveGroupedFlag {
			...
			int group;
			FDS_FontUsFlag(int group){ this.group = group;}
	...
	/** MO:DCA, page 378.<br><br> The Font Coded Graphic Character Set Global Identifier triplet is used to specify the code page and character set for a coded font. */
	...
	public static class ResourceObjectType extends Triplet {
		...
		public static enum ROT_ObjectType{
			...
			int code;
			ROT_ObjectType(int code){ this.code = code;}
	 ...
	 * products: Pre-year 2012 AFP applications.<br>
	 * <br>
	 * The Object Function Set Specification triplet is used to specify the Object Content
	 * Architecture (OCA) level for objects in a MO:DCA document.
	 */
	public static class ObjectFunctionSetSpecification_Retired extends Triplet {
		...
		public static enum OCAFunctionSet{
			...
			IOCA_FS10(0x8000);
			int code;
			OCAFunctionSet(int code){this.code=code;}
	 ...
	 * MODCA page 381.<br><br>
		...
		resource name by a map structured field, such as a Map Media Type structured
		field.
	 */
	public static class ExtendedResourceLocalIdentifier extends Triplet {
		...
		public static enum ERLI_ResourceType{
			...
			public static ERLI_ResourceType valueOf(byte codeByte) throws AFPParserException{
				...
				else throw new AFPParserException(ERLI_ResourceType.class.getSimpleName()+": resource type code 0x"+Integer.toHexString(codeByte)+ " is unknown.");
			}
			public int toByte(){
	...
	/**
	 * MODCA page 383.<br><br>
	 * The Resource Local Identifier triplet may be used to specify a resource type and a
	 * one-byte local identifier or LID. The LID usually is associated with a specific
	 * resource name by a map structured field, such as a Map Coded Font structured
	 * field.
	 */
	public static class ResourceLocalIdentifier extends Triplet {
		...
		public static enum RLI_ResourceType{
			...
			public static RLI_ResourceType valueOf(byte codeByte) throws AFPParserException {
				...
				else throw new AFPParserException(RLI_ResourceType.class.getSimpleName() + ": resource type code 0x" + Integer.toHexString(codeByte) + " is unknown."); 
			}
			public int toByte(){
	...
	/**
	 * MODCA page 385.<br><br>
	 * The Resource Section Number triplet specifies a coded font section number. It may
	 * be used to select a single section of a double-byte coded font if less than the entire
	 * double-byte coded font is required for processing. For a description of coded fonts
	 * see the Font Object Content Architecture Reference.
	 */
	...
	/**
	 * MODCA page 386.<br><br>
	 * The Character Rotation triplet is  used to specify character rotation relative to the
	 * Character coordinate system. See  the Font Object Content 
	 * Architecture Reference for further information.
	 */
	...
	/**
	 * MODCA page 387.<br><br>
	 * The Object Byte Offset triplet is used to specify the byte offset of an indexed object
	 * within a document.
	 */
	public static class ObjectByteOffset extends Triplet {
		...
		public void writeAFP(OutputStream os, AFPParserConfiguration config) throws IOException {
			...
			if(byteOffsetHighOrder!=null) os.write(UtilBinaryDecoding.longToByteArray(byteOffsetHighOrder, 4));
		}
	}

	/**
	 * MODCA page 388.<br><br>
	...
	/**
	 * MODCA page 389.<br><br>
	 * The Descriptor Position triplet is used to associate an Object Area Position 
	 * structured field with an Object Area Descriptor structured field.
	 */
	...
	/**
	 * MODCA page 390.<br><br>
	 * The Media Eject Control triplet is used to specify the type of media eject that is
	 * performed and the type of controls that are activated when a new medium map
	 * is invoked and N-up partitioning is specified.
	 */
	public static class MediaEjectControl extends Triplet {
		...
		public static enum MediaEjectControlType{
			...
			public static MediaEjectControlType valueOf(byte codeByte) throws AFPParserException{
				...
				else throw new AFPParserException(MediaEjectControlType.class.getSimpleName() + ": code byte 0x" + Integer.toHexString(codeByte) + " is undefined."); 
			}
			public int toByte(){
	...
	/**
	 * MODCA page 576.<br><br>
	 * The use of this triplet is restricted to products that generate or process the retired
	 * MO:DCA interchange set MO:DCA IS/2.
	 * The Page Overlay Conditional Processing triplet is used to identify the intended
	 * utilization of a page overlay as produced by a generator. This triplet can also be
	 * used to define an overlay level that determines whether the overlay is to be
	 * processed.
	 */
	...
	/**
	 * MODCA page 578.<br><br>
	 * The use of this triplet is restricted to products that generate or process the retired
		MO:DCA interchange set MO:DCA IS/2.
		The Resource Usage Attribute triplet can be used for resource management. It is
		used with the Include Page Overlay and Map Page Overlay structured fields to
		identify the approximate frequency with which an associated page overlay is
		processed. This is indicated by assigning either a low or high value to this triplet.
		The Resource Usage Attribute triplet has no processing semantics associated with
		it.
	 */
	 ...
	 * 
The use of this triplet is restricted to the BMO and BPS structured fields in external
(print file level) AFP resource groups for the following products:
v PSF/MVS
v PSF/VSE
v RPM 2.0
v RPM 3.0
v PSF/2 (DPF)
v RMARK
The Object Checksum object specifies a qualifier that can be used to identify or fingerprint an object.
	 *
	 */
	public static class  ObjectChecksum extends Triplet {
		...
		public static enum CheckSumFormat{
			...
			public static CheckSumFormat valueOf(byte codeByte) throws AFPParserException {
				...
				else  throw new AFPParserException(CheckSumFormat.class.getSimpleName() + ": checksum format code 0x" + Integer.toHexString(codeByte) + " is undefined."); 
			}
			public int toByte(){
		...
		public static enum ChecksumFlag implements IMutualExclusiveGroupedFlag{
			...
			int group;
			ChecksumFlag(int group){this.group = group; }
	...
	/**
	 * MODCA page 581. Retired function<br><br>
	 * The use of this triplet is restricted to the BMO and BPS structured fields in external
		(print file level) AFP resource groups for the following products:
		v PSF/MVS
		v PSF/VSE
		...
		v PSF/2
		v RMARK
		The Object Origin Identifier triplet is used to identify the system on which an
		object originated.
	 */
	 ...
	 *
		The use of this triplet is restricted to the IMM structured field for the following
		products:
		v AFP OnDemand
		v AFP Workbench
		The IMM Insertion triplet is used to indicate that the Invoke Medium Map (IMM)
		structured field on which it is specified was inserted at the beginning of a page
		group by a filtering application. The IMM was inserted between the BNG and the
		first BPG in the group, but only if an IMM was not already specified there. The
		purpose of the inserted IMM is to allow the page group to be processed in
		standalone fashion. This triplet is ignored by presentation servers, and the IMM on
		which it is specified is processed as if the triplet were absent. The presence of this
		triplet on an IMM may be used by an inverse filtering application to remove the
		IMM when it is desired to present the complete document as it appeared before
		the IMM was inserted.
	 */
...
The use of this triplet is restricted to the MCF-2 structured field for 3800
compatibility for the following products:
v PSF/MVS
v PSF/VM
v PSF/VSE
v PSF/400
v PSF/2
v Infoprint Manager (IPM)
v 3800 printer
v Applications that generate MCF-2s in documents to be printed on the 3800
printer
The Text Orientation triplet is used to specify the text orientation for a coded font.
When the MCF-2 structured field is used to reference different sections of the same
double-byte font, a Text Orientation (X'1D') triplet may be specified in any of the
repeating groups associated with the font and need only be specified in one of the
repeating groups. However, if specified in more than one of the associated
repeating groups, the value of all Text Orientation (X'1D') triplets must be identical.
	 */
	...
	/**
	MODCA page 572, retired function.<br><br>
	The use of this triplet is restricted to the BBC, BGR, BII, BIM, IPS structured fields
	for the migration of line-data containing bar code objects, graphic objects, image
	objects, and page segments to MO:DCA document format. This triplet may be
	specified on these structured fields only for objects that occur directly in a page.
	The triplet may not be specified on objects in a resource group or in a resource
	...
	public static class LineDataObjectPositionMigration extends Triplet {
		...
		public static enum LocationAndOrientation{
			...
			public static LocationAndOrientation valueOf(byte codeByte) throws AFPParserException {
				for(LocationAndOrientation lao : values()) if(lao.ordinal()==codeByte) return lao;
	...
	/**
	 * MODCA, page 395.<br><br>
		The Measurement Units triplet is
		used to specify the units of measure for a presentation space.
	 */
	...
	/**
MODCA, page 396.<br><br>
The Object Area Sizeand Y directions.
triplet is used to specify theextent of an object area in the X
	 */
	...
	/**
	MOIDCA, page 397<br><br>
	The Area Definition triplet is used to define the position and size of a rectangular
	area on a document component presentation space. The document component may
	be a page or overlay, in which case the area is defined on the page or overlay
	presentation space, or it may be a data object, in which case the area is defined on
	the object area presentation space.
	 */
	...
	/**
	MODCA, page 398.<br><br>
	The Color Specification triplet is used to specify a color value and defines the color
	space and encoding for that value.
	 */
	...
	/**
		MODCA, page 403.<br><br>
		The Encoding Scheme ID triplet is used to specify the encoding scheme associated
		with a code page. It may optionally also specify the encoding scheme for the user
		data.
	 */
	public static class EncodingSchemeID extends Triplet {
		...
		public static enum EncodingScheme implements IMutualExclusiveGroupedFlag{
			...
			int group;
			EncodingScheme(int code){ this.group=code;}
			...
			public static EnumSet<EncodingScheme> valueOf(int code) throws AFPParserException{
				...
				else throw new AFPParserException(EncodingScheme.class.getSimpleName() + ": basic encoding value 0x" + Integer.toHexString(basicEncoding) + " is undefined.");
				...
				else throw new AFPParserException(EncodingScheme.class.getSimpleName() + ": number of bytes value 0x" + Integer.toHexString(numberOfBytes) + " is undefined.");
		...
		/**
		 * Sets the given {@link EncodingScheme} and un-sets the mutual exclusive {@link EncodingScheme} values.
		 * @param encodingScheme.
		 */
		public void setEncodingSchemeForCodePage(EncodingScheme encodingScheme) {
			if(encodingSchemeForCodePage == null) encodingSchemeForCodePage = EnumSet.noneOf(EncodingScheme.class);
	...
	/**
	 * MODCA, page 406.<br><br>
	 * The Medium Map Page Number triplet is used to specify the sequence number of
		the page in the set of sequential pages whose presentation is controlled by the
		most recently activated medium map.
	 */
	 ...
	 * 
	 * The Object Structured Field Extent triplet is used to specify the number of
		structured fields contained in an object, starting with the Begin Object structured
		field and ending with the End Object structured field.
	 */
	public static class ObjectStructuredFieldExtent extends Triplet {
		...
		public void decodeAFP(byte[] sfData, int offset, int length, AFPParserConfiguration config) throws AFPParserException {
			...
			if(this.length>6)  numberOfSFHigh = UtilBinaryDecoding.parseLong(sfData, offset +6, 4);
		...
		public void writeAFP(OutputStream os, AFPParserConfiguration config) throws IOException {
			...
			if(numberOfSFHigh!=null) os.write(UtilBinaryDecoding.longToByteArray(numberOfSFHigh, 4));
		}
	} 
	 ...
	 * 
	 * The Object Offset triplet specifies the number of objects of a particular type that
		precede a selected object in the document. If the object being counted is a
		document, this triplet specifies the number of documents that precede the selected
		object in the print file.
	 */
	public static class ObjectOffset extends Triplet {
		...
		public void decodeAFP(byte[] sfData, int offset, int length, AFPParserConfiguration config) throws AFPParserException {
			...
			if(this.length>7)  nrOfPrecedingObjectsHigh = UtilBinaryDecoding.parseLong(sfData, offset +8, 4);
		...
		public void writeAFP(OutputStream os, AFPParserConfiguration config) throws IOException {
			...
			if(nrOfPrecedingObjectsHigh!=null) os.write(UtilBinaryDecoding.longToByteArray(nrOfPrecedingObjectsHigh, 4));
		}
		...
		public static enum ObjectType{
			...
			public static ObjectType valueOf(short codeByte) throws AFPParserException {
				...
				else throw new AFPParserException(ObjectType.class.getSimpleName()+ ": object type 0x" + Integer.toHexString(codeByte) + " is undefined."); 
			}
			public int toByte(){
	...
	/**
	 * MODCA, page 414.<br><br>
	 * The Object Count triplet specifies the number of subordinate objects of a particular
		type contained in an object.
	 */
	...
	public static class LocalObjectDateAndTimeStamp extends Triplet {
		...
		public static enum DateAndTimeStampType{
			...
			public static DateAndTimeStampType valueOf(byte codeByte) throws AFPParserException {
				...
				else throw new AFPParserException(DateAndTimeStampType.class.getSimpleName() + ": type 0x" + Integer.toHexString(codeByte) + " is undefined.");
			}
			public int toByte(){
	...
	/**
	 * MODCA, page 419.
	 * The Medium Orientation triplet may be used to specify the orientation of the
	...
	/**
	 * MODCA, page 421.<br><br> 
	 * The Resource Object Include triplet identifies an object to be included on a
		presentation space at a specified position.
	 */
	...
	/**
	 * MODCA, page 423.<br><br>
	 * This triplet is used to specify the resulting appearance 
	 * when data in a new presentation space is merged with data 
	 * in an existing presentation space.
	 */
	 ...
	 * 
	 * This triplet is used to specify the rules for establishing the color attribute of areas
		formed by the intersection of two presentation spaces. It is specified on structured
		fields associated with a presentation space that is to be merged onto an existing
		presentation space.
	 */
	...
	/**
	 * MODCA, page 427.<br><br>
	 * The Universal Date and Time Stamp triplet specifies a date and time in accordance 
	 * with the format defined in ISO 8601: 1988 (E).
	 */
	 ...
	 * 
	 * The Toner Saver triplet activates a toner saver mode for printing. The toner saver
		control specified by this triplet overrides any other toner saver controls that may
		be active in the printer.
	 */
	public static class TonerSaver extends Triplet {
		...
		public static enum TonerSaverFunction{
			...
			public static TonerSaverFunction valueOf(short codeByte) throws AFPParserException{
				...
				else throw new AFPParserException(TonerSaverFunction.class.getSimpleName()+ ": tonser saver function code 0x" + Integer.toHexString(codeByte) + " is undefined.");
			}
			public int toByte(){
	...
	public static class ColorFidelity extends Triplet {
		...
		public static enum ExceptionContinuationRule{
			...
			public static ExceptionContinuationRule valueOf(byte ruleByte) throws AFPParserException{
				for(ExceptionContinuationRule ecr : values()) if (ecr.ordinal()+1 == ruleByte) return ecr;
		...
		public static enum ExceptionReportingRule{
			...
			public static ExceptionReportingRule valueOf(byte ruleByte) throws AFPParserException{
				for(ExceptionReportingRule ecr : values()) if (ecr.ordinal()+1 == ruleByte) return ecr;
		...
		public static enum ExceptionSubstitutionRule{
			...
			public static ExceptionSubstitutionRule valueOf(byte ruleByte) throws AFPParserException{
				for(ExceptionSubstitutionRule ecr : values()) if (ecr.ordinal()+1 == ruleByte) return ecr;
	...
	/**
	 * MODCA, page 435. <br><br>
	 * The Font Fidelity triplet is used to specify the exception continuation rules for font
	...
	/**
	 * MODCA, page 437.<br><br>
	 * The Page Position Information triplet is used to tag a page with the Page Position
		(PGP) structured field repeating group information that is used to present the
		page. The PGP is specified in the medium map referenced by the FQN type
		X'8D'—Begin Medium Map Reference triplet. This information is used for viewing
		the page with a particular form map, which is normally the form map that the
		document containing this page was archived with.<br><br>
	 ...
	 *
	 * The Parameter Value triplet is used to pass parameter values to an executable
	 * program such as an object handler or a system command interpreter.
	 */
	...
	public static class PresentationControl extends Triplet {
		...
		public static enum PresentationControlFlags implements IMutualExclusiveGroupedFlag{
			...
			int group;
			PresentationControlFlags(int group){this.group=group;}
		...
		public void setPresentationControlFlag(PresentationControlFlags presentationControlFlag) {
			...
			if(presentationControlFlags==null) presentationControlFlags = EnumSet.noneOf(PresentationControlFlags.class);
	 ...
	 *
	 * The Font Resolution and Metric Technology specifies certain metric characteristics
		of a FOCA raster-technology font character set which may have affected the
		formatting of the document with this font. This information, as carried by the X'84'
		triplet, may be used by presentation servers and presentation devices to select the
		best-matching coded font for presentation.
	 */
	public static class FontResolutionAndMetricTechnology extends Triplet {
		...
		public static enum MetricTechnology{
			...
			public static MetricTechnology valueOf(byte codeByte) throws AFPParserException{
				...
				else throw new AFPParserException(MetricTechnology.class.getSimpleName()+": technology code 0x"+ Integer.toHexString(codeByte) + " is undefined.");
			}
			public int toByte(){ return ordinal()+1;}
		}
	} 
	...
	public static class FinishingOperation extends Triplet {
		...
		public static enum ReferenceCorner{
			...
			int code;
			ReferenceCorner(int code){this.code=code;}
	 ...
	 * 
	 * The Text Fidelity triplet is used to specify the exception continuation and reporting
		rules for text exceptions. A text exception is detected when an unrecognized or
		unsupported text control sequence is encountered in a PTOCA text object.
	 */
	 ...
	 * 
	 * The Media Fidelity triplet is used to specify the continuation rule if a request for a
		specific media or a specific media bin cannot be satisfied.
	 */
	 ...
	 * 
	 * The Finishing Fidelity triplet is used to specify the exception continuation and
		reporting rules for finishing exceptions. A finishing exception is detected when the
		specified finishing operation cannot be satisfied.
	 */
	...
	/**
	 * MODCA, page 456.<br><br>
	 * The Data-Object Font Descriptor triplet is used to specify the parameters needed to
		render a data-object font. Data-object fonts are non-FOCA font resources, such as
		TrueType and OpenType fonts. An MDR structured field is used to map a
		data-object font as a resource.
	 */
	public static class DataObjectFontDescriptor extends Triplet {
		...
		public static enum FontInformationFlag implements IMutualExclusiveGroupedFlag{
			...
			FontInformationFlag(int group){ this.group=group;}
	 ...
	 * 
	 * The Locale Selector triplet is used to identify the end-user community for
		presentation text data. The locale information consists of an ISO-639 based
		language code, an ISO-15924 based script code, an ISO-3166 based region code,
		and an application-specific variant code. The encoding for all four parameters is
		UTF-16BE. Additional information on these parameters can be found at the
		following urls:
		<ul>
		<li>The definition of language codes can be found at http://lcweb.loc.gov/standards/
		iso639-2/iso639jac.html
		<li> The definition of script codes can be found at http://www.unicode.org/reports/tr24
		<li> The definition of region codes can be found at http://www.iso.org/iso/en/prods-
	...
	public static class LocaleSelector extends Triplet {
		...
		public static enum LocalSelectorFlag implements IMutualExclusiveGroupedFlag{
			...
			int group;
			LocalSelectorFlag(int group){this.group=group;}
			public int getGroup(){ return group; }
	...
	/**
	 * MODCA, page 461.<br><br>
	 * The UP3i Finishing Operation triplet is used to specify finishing operations that are
		to be applied to media. More specifically, this triplet is a carrier for finishing
		operations and parameters that are defined by the UP3i consortium in the UP3i
		Specification.
	*/
	 ...
	 * 
	 * The Color Management Resource Descriptor triplet specifies the processing mode 
	 * and scope for a Color Management Resource (CMR).
	 */
	 ...
	 *
	 * The Rendering Intent triplet specifies the rendering intent parameter, which is used
		to modify the final appearance of color data. This parameter is based on the
		rendering intents defined by the International Color Consortium (ICC). For more
		information on rendering intents, see the International Color Consortium Specification
		ICC.x, File Format for Color Profiles.
	 */
	 ...
	 * 
	 *The CMR Tag Fidelity triplet is used to specify the exception continuation and
		reporting rules for Color Management Resource (CMR) tag exceptions. A CMR tag
		exception is detected when an unsupported CMR tag is encountered in a Color
	 ...
	 * 
	 * The Device Appearance triplet specifies one of a set of architected appearances to
	...
	/**
	 * MODCA, page 476.<br><br>
	 * The Object Container Presentation Space Size triplet specifies the presentation
		space size, or how such a size is determined, for certain container object types.
	 */
	public static class ObjectContainerPresentationSpaceSize extends Triplet {
		...
		public static enum PDFPresentationSpace{
			...
			public static PDFPresentationSpace valueOf(byte codeByte) throws AFPParserException{
				for(PDFPresentationSpace ps : values()) if (ps.ordinal()+1 == codeByte) return ps;
	...
	public static enum GlobalID_Use{
		...
		int code;
		GlobalID_Use(int code){this.code=code;}
	...
	public static enum GlobalID_Format{
		...
		URL(0x20);
		int code;
		GlobalID_Format(int code){ this.code=code;}
