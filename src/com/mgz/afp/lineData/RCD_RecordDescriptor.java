...
/**
 *  *  Programming Guide and Line Data Reference(ha3l3r04.pdf), page 127.<br>
 *  <br>
The Record Descriptor structured field contains information, such as record
position, text orientation, font selection, field selection, and conditional processing
identification, used to format line data that consists of records tagged with record
identifiers.
 *
 */
public class RCD_RecordDescriptor extends StructuredFieldBaseTriplets {
	public static enum RCD_Flag{
		...
		int group;
		RCD_Flag(int group){ this.group=group;}
		...
		public static EnumSet<RCD_Flag> valueOf(int flagBytes){
			...
			if((flagBytes & 0x000020)!=0) result.add(ResetPageNumber_ResetPageNumberToSpecifiedValue);
		...
		public static byte[] toBytes(EnumSet<RCD_Flag> flags){
			...
			// 0x800000 is not used.
			// 0x400000 is not used.
			if(flags.contains(GenerateInlinePosition_NewPosition)) result|= 0x200000; // (flagBytes & 0x200000)!=0) result.add(GenerateInlinePosition_NewPosition);
			if(flags.contains(GenerateBaselinePosition_NewPosition)) result|= 0x100000; // (flagBytes & 0x100000)!=0) result.add(GenerateBaselinePosition_NewPosition);
			if(flags.contains(GenerateFontChange_AsSpecified)) result|= 0x080000; // (flagBytes & 0x080000)!=0) result.add(GenerateFontChange_AsSpecified);
			if(flags.contains(GenerateSuppression_TextIsSuppressible)) result|= 0x040000; // (flagBytes & 0x040000)!=0) result.add(GenerateSuppression_TextIsSuppressible);
			if(flags.contains(FieldRCD_FieldRCD)) result|= 0x020000; // (flagBytes & 0x020000)!=0) result.add(FieldRCD_RecordRCD);
			if(flags.contains(UseFixedData_DoPresent)) result|= 0x010000; // (flagBytes & 0x010000)!=0) result.add(UseFixedData_DoPresent);
			// 0x008000 is not used.
			// 0x004000 is not used.
			// 0x002000 is not used.
			if(flags.contains(ConditionalProcessing_DoPerformCP)) result|= 0x001000; // (flagBytes & 0x001000)!=0) result.add(ConditionalProcessing_DoNotPerformCP);
			// 0x000800 is not used.
			if(flags.contains(RelativeBaselinePosition_RelativePosition)) result|= 0x000400; // (flagBytes & 0x000400)!=0) result.add(RelativeBaselinePosition_AbsolutePosition);
			// 0x000200 is not used.
			// 0x000100 is not used.
			if(flags.contains(NewPage_LogicalPageEject)) result|= 0x000080; // (flagBytes & 0x000080)!=0) result.add(NewPage_NoEffect);
			if(flags.contains(PrintPageNumber_PrintPageNumber)) result|= 0x000040; // (flagBytes & 0x000040)!=0) result.add(PrintPageNumber_NoEffect);
			if(flags.contains(ResetPageNumber_ResetPageNumberToSpecifiedValue)) result|= 0x000020; // (flagBytes & 0x000020)!=0) result.add(ResetPageNumber_NoEffect);
			if(flags.contains(GroupIndicator_SaveGroupHeader)) result|= 0x000010; // (flagBytes & 0x000010)!=0) result.add(GroupIndicator_InputDataIsNotPartOfAGroup);
			if(flags.contains(FieldDelimeterSize_2Bytes)) result|= 0x000008; // (flagBytes & 0x000008)!=0) result.add(FieldDelimeterSize_1Byte);
			if(flags.contains(UseRecordID_SelectRecordID)) result|= 0x000004; // (flagBytes & 0x000004)!=0) result.add(UseRecordID_DoNotSelectRecordID);
			// 0x000002 is not used.
			// 0x000001 is not used.
