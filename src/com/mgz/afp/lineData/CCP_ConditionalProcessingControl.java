...
/**
 * Programming Guide and Line Data Reference (ha3l3r04.pdf), page 83<br>
 * <br>
 * The Conditional Processing Control structured field defines tests to be performed
 * on selected input records in line data and specifies the actions to take based on the
 * test results. This optional structured field is selected with LND, RCD or XMD
 * structured fields in the Page Definition. An LND, RCD or XMD can have a unique
 * CCP associated with it or it can reference a CCP that has already been used. In
 * either case, the CCP is referenced with the CCPID field of the LND, RCD or XMD.
 * If a CCP structured field is included in a Page Definition, it must appear before the
 * Data Maps in the Page Definition.
 */
public class CCP_ConditionalProcessingControl extends StructuredField {
	public static enum CCP_Flag{
		...
		SpacingActions(0x20);
		int code;
		CCP_Flag(int code){this.code=code;}
	...
	/**
	 * Programming Guide and Line Data Reference (ha3l3r04.pdf), page 84<br>
	 * <br>
	 * Each repeating group of the CCP contains action information. See Table 9 for the
	 * definitions of the CCP repeating groups.
	 */
	public static class CCP_RepeatingGroup implements IAFPDecodeableWriteable{
		public static enum CCP_TimingOfAction{
			...
			int code;
			CCP_TimingOfAction(int code){ this.code=code;}
		...
		public void decodeAFP(byte[] sfData, int offset, int length, AFPParserConfiguration config) throws AFPParserException {
			int actualLength = getActualLength(sfData, offset, length);
			if(actualLength>=1) timingOfAction = CCP_TimingOfAction.valueOf(UtilBinaryDecoding.parseInt(sfData, offset, 1));
			else timingOfAction = null;
			if(actualLength>=2) mediumMapAction = CCP_MediumMapAction.valueOf(sfData[offset +1]);
			else mediumMapAction = null;
			if(actualLength>=10) mediumMapName = new String(sfData, offset +2, 8, config.getAfpCharSet());
			...
			if(actualLength>=19) dataMapName = new String(sfData, offset +11, 8, config.getAfpCharSet());
			else dataMapName = null;
			if(actualLength>=20) comparison = CCP_Comparison.valueOf(sfData[offset + 19]);
			else comparison = null;
			if(actualLength>=21) comparisonString = new String(sfData, offset +20, actualLength -20, config.getAfpCharSet());
